...
## HARN-013 Enterprise Integration

ERP、供应商和物流调用必须经过类型化适配器与 Tool Gateway。服务地址只能来自服务端配置；非回环地址必须使用 HTTPS。请求必须携带租户和合同版本；写请求还必须携带幂等键与审批 subject hash。超时、429 和 5xx 可分类为临时失败，4xx、租户错配和响应 Schema 错误必须永久失败并停止流程。

## HARN-014 Attachment Bundle and Task Retention

单任务附件必须受文件数、单文件大小、总大小、扩展名和租户对象存储边界约束。重复业务行可以合并，但每个文件的证据必须保留；同一物料的数量、单位或设备型号冲突时必须停止并请求人工确认。工作台删除采用软归档，待处理 Job 进入终止状态，附件、证据、审批、PO 和追加式审计禁止级联物理删除。
